...
public class JamaEquationsSystem extends EquationsSystem {
    private static Logger logger = Logger
            .getLogger(JamaEquationsSystem.class.getName());
    ...
    public EquationsSystem assemble(EquationsSystem systeme) {
        ...
        assemblage.setNonNullValues(
                this.getNonNullValues() + systeme.getNonNullValues());

        // on assemble les matrices d'observation
        assemblage.observations = new Matrix(
                this.getObsRowNumber() + systeme.getObsRowNumber(), 1);
        ...
        // présentes
        assemblage.setConstraints(
                new Vector<LSConstraint>(this.getConstraints()));
        ...
        int nbColonnes = this.getColumnNumber() + systeme.getColumnNumber()
                - 2 * setInconnues.size();
            ...
            for (int j = 0; j < nbColonnes; j++) {
                ...
                if (i < this.matriceA.getRowDimension()) {
                    ...
                    if (!(j + 1 >= assemblage.getUnknowns().size())) {
                        if (inconnue
                                .equals(assemblage.getUnknowns().get(j + 1))) {
                            estX = true;
                        }
                    }
                    ...
                    // on détermine la colonne correspondant dans
                    // systeme.matriceA
                    ...
                    assemblage.matriceA.set(i, j, systeme.getA(
                            i - this.matriceA.getRowDimension(), colonne));
    ...
    public void ajustementMoindresCarres(Map<String, Double> poids) {
        ...
        Matrix matriceAInverser = this.matriceA.transpose()
                .times(matricePoids.times(this.matriceA));
        ...
        // System.out.println(String.valueOf(matriceAInverser.getRowDimension()));
        // System.out.println(String.valueOf(matriceAInverser.getColumnDimension()));
        // System.out.println(matriceAInverser.det());
        // on termine l'ajustement
        logger.fine("inversion de la matrice");
        // this.observations.print(2, 1);
        // matriceAInverser.print(2, 1);
        ...
        Matrix matriceResidus = this.matriceA.times(solution)
                .minus(this.observations);
     ...
     * carrée de dimension le nb de ligne de la matrice Jacobienne A. Le poids
     * est placé dans la diagonale.
     * 
     */
