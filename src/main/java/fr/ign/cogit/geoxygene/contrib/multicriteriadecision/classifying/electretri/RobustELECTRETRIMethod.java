...
public class RobustELECTRETRIMethod extends ELECTRETRIMethod {
  ...
  private static Logger logger = Logger
      .getLogger(RobustELECTRETRIMethod.class.getName());
   ...
   *                     elementType="java.util.Map" qualifier=
   *                     "crits0:java.util.Set java.util.Set"
   */
  ...
  private void decision(Map<String, Double> votes, Set<Criterion> criteres,
    ...
    for (Double valCoupe : this.seuilCoupeSet) {
      ...
      decision = this.decisionELECTRETRI(criteres, valeursCourantes,
          conclusion);
    ...
    while (i.hasNext()) {
      ...
      for (Set<Criterion> criteria : map.keySet()) {
        if (param.equals("Poids")) {
          this.setPoids(criteria,
              map.get(criteria).iterator().next().doubleValue());
        }
        if (param.equals("Veto")) {
          this.setVeto(criteria,
              map.get(criteria).iterator().next().doubleValue());
        }
        if (param.equals("Preference")) {
          this.setPreference(criteria,
              map.get(criteria).iterator().next().doubleValue());
        }
        if (param.equals("Indifference")) {
          this.setIndifference(criteria,
              map.get(criteria).iterator().next().doubleValue());
        }
      }
    }
